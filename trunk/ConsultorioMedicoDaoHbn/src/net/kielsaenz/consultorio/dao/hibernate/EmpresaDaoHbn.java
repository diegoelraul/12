...
import net.kielsaenz.consultorio.dao.hibernate.HibernateUtil;
...
public class EmpresaDaoHbn implements EmpresaDao {
	...
	public Empresa getEmpresaPorRUC(String ruc) throws DaoException {
		...
		try {
			...
			htx = hs.beginTransaction();
			Query hqlQuery = hs
					.createQuery("select e from Empresa e where e.ruc = :ruc");
	...
	public List<Empresa> getEmpresaPorRazonSocial(String razonSocial,
			boolean aplicarLike, int tipo) throws DaoException {
		if (razonSocial == null) {
	...
	public List<Empresa> getEmpresas() throws DaoException {
		...
		try {
			...
			htx = hs.beginTransaction();
			Query hqlQuery = hs
					.createQuery("from Empresa e order by e.razonSocial asc");
			empresas = hqlQuery.list();
			htx.commit();
		} catch (HibernateException e) {
			if (htx != null && htx.isActive()) {
